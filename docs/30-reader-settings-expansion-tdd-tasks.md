...
- [ ] **T-1.1.7** Registry 注册 `color_temperature`
...
- [ ] **T-1.3.6** `onCleared()` 取消 Job
  - 测试：ViewModel 销毁后 `eyeCareCheckJob` 已取消
  - 实现：`eyeCareCheckJob?.cancel()`

- [ ] **T-1.3.7** `dismissEyeCareReminder()` 重启 checker（v5.1 §2.1.3 补充）
...
- [ ] **T-1.5.2** PORTRAIT 设置 `SCREEN_ORIENTATION_PORTRAIT`
  - 测试：`orientationLock = PORTRAIT` 时 `activity.requestedOrientation` 为对应常量
  - 实现：`LaunchedEffect`

- [ ] **T-1.5.3** 退出阅读页恢复 SYSTEM
...
- [ ] **T-2.1.6** 性能：wordSpacing reflow ≤ 35ms
...
- [ ] **T-2.3.3** WindowInsets 叠加到 marginBottom
...
- [ ] **T-2.4.2** PageDelegate 拖拽优先级高于自定义手势
  - 测试：翻页动画拖拽中，点击左上区域，`onAction` 不被调用
  - 实现：`PageDelegate.onTouch() == true` 时拦截

- [ ] **T-2.4.3** 文本选区激活时长按不触发 longPress action
...
- [ ] **T-3.4.7** 性能：10 条规则 + 100KB 文本 ≤ 20ms
  - 测试：benchmark
  - 实现：优化
...
- [ ] **T-3.7.8** 性能：双页渲染 ≤ 14ms
  - 测试：benchmark
  - 实现：优化

- [ ] **T-3.7.9** `renderPageRegion()` 仅绘制 shell + canvas（v5.1 §2.3.3 修正）
...
- [ ] **T-4.1.6** `bionicSegments` 缓存在 TextLine 中
  - 测试：分页后 `TextLine.bionicSegments != null`（当 bionicReading = true）
  - 实现：分页阶段赋值

- [ ] **T-4.1.7** 渲染使用 segment 绘制
...
- [ ] **T-4.1.8** 性能：Bionic Reading 渲染 ≤ 15ms
  - 测试：benchmark
...
- [ ] **T-4.2.2** `BreakIterator.getLineInstance()` 返回断字点
...
- [ ] **T-4.2.6** 性能：断字 reflow ≤ 40ms
  - 测试：benchmark
  - 实现：优化

- [ ] **T-4.2.7** `BreakIterator.getLineInstance()` 效果验证（v5.1 §2.2.4 局限性说明）
...
- [ ] **T-4.3.8** `canvas.save/restore` 配对
...
| Phase 1：零 Reflow 增量 | 27 | 1 周 | ⬜ 待开始 |
| Phase 2：Reflow 增量 | 22 | 2 周 | ⬜ 待开始 |
| Phase 3：中等复杂度 | 31 | 3 周 | ⬜ 待开始 |
| Phase 4：高复杂度 | 26 | 4 周 | ⬜ 待开始 |
| 附录 A：性能基准 | 9 | — | ⬜ 待开始 |
| 附录 B：降级策略 | 5 | — | ⬜ 待开始 |
| 附录 C：向后兼容 | 5 | — | ⬜ 待开始 |
| 附录 D：迁移与回滚（v5.1） | 7 | — | ⬜ 待开始 |
| 附录 E：测试策略验证（v5.1） | 7 | — | ⬜ 待开始 |
| **总计** | **253** | **14 周** | **96/253 完成** |
