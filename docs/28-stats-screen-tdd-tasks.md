...
- [ ] **T-1.1.1** `ReadingSessionEntity` 默认值验证
...
- [ ] **T-1.1.2** `ReadingSessionEntity` 字段完整性
...
- [ ] **T-1.11.1** `ReadingProgressEntity` 不再含 `readTime`
  - 测试：编译通过（`ReadingProgressEntity` 无 `readTime` 属性），已有 `BookEntityTest` 不受影响
  - 实现：删除 `readTime: Long` 字段

- [ ] **T-1.11.2** `ChapterReadingStatsEntity` 不再含 `readTimeSeconds`
  - 测试：编译通过（无 `readTimeSeconds` 属性）
  - 实现：删除 `readTimeSeconds: Long` 字段

- [ ] **T-1.11.3** `ReadingHistoryEntity` 不再含 `readingDurationMinutes`
...
- [ ] **T-1.12.1** `updateProgress()` 不含 `readTime` 参数
  - 测试：编译通过，调用 `updateProgress()` 时无 `readTime` 参数
  - 实现：移除 `readTime` 参数

- [ ] **T-1.12.2** 移除 `getReadingDurationByBookId()`
  - 测试：编译通过，无此方法引用
  - 实现：删除方法

- [ ] **T-1.12.3** 移除 `getAllReadingDurations()`
  - 测试：编译通过
  - 实现：删除方法

- [ ] **T-1.12.4** 移除 `getTodayTotalReadingTime()`
  - 测试：编译通过
  - 实现：删除方法

### 1.13 ChapterReadingStatsDao 改造

- [ ] **T-1.13.1** 移除 `addReadTime()` / `addReadTimeOrCreate()`
  - 测试：编译通过，无方法引用
  - 实现：删除方法

- [ ] **T-1.13.2** `ensureExists()` 不再引用 `readTimeSeconds`
...
- [ ] **T-2.1.1** `flushChapterTime()` 插入 ReadingSessionEntity
...
- [ ] **T-3.1.1** `pauseReadingSession()` 调用 `flushChapterTime()`
  - 测试：Mock `BookSessionManager`，调用 `pauseReadingSession()` 后验证 `flushChapterTime()` 被调用
  - 实现：追加 `bookSessionManager.flushChapterTime()`

- [ ] **T-3.1.2** `pauseReadingSession()` 调用 `readingStateManager.pauseSession()`
  - 测试：验证 `pauseSession()` 被调用
  - 实现：保持已有调用

### 3.2 resumeReadingSession()

- [ ] **T-3.2.1** `resumeReadingSession()` 调用 `resetChapterStartTimestamp()`
...
- [ ] **T-3.2.2** `resumeReadingSession()` 调用 `readingStateManager.resumeSession()`
...
- [ ] **T-4.1.1** 书架每本书时长读 `ReadingSessionDao.getBookTotals()`
...
- [ ] **T-4.1.2** 书架今日总时长读 `ReadingSessionDao.getTodayTotal()`
  - 测试：Mock 返回 1800L（秒），验证 UI 显示 "30m"
  - 实现：替换 `getTodayReadingTime()`

- [ ] **T-4.1.3** `BookshelfViewModel` 不再依赖 `ReadingProgressRepository.getReadingDurations()`
  - 测试：`verify(exactly = 0) { readingProgressRepository.getReadingDurations() }`
  - 实现：移除旧依赖

- [ ] **T-4.1.4** 今日时长格式化使用 `StatsFormatter`
...
- [ ] **T-4.2.1** 章节列表时长读 `ReadingSessionDao.getChapterTotals()`
  - 测试：Mock 返回章节时长数据，验证列表正确显示
  - 实现：替换 `ChapterReadingStatsDao.getStatsByBookId()`

### 4.3 ReadingProgressRepository 迁移

- [ ] **T-4.3.1** `getReadingDuration()` 委托 `ReadingSessionDao.getBookTotals()`
  - 测试：Mock DAO 返回数据，验证 Repository 正确透传
  - 实现：委托查询

### 4.4 BookItem 格式化迁移

- [ ] **T-4.4.1** `BookItem.kt` 使用 `StatsFormatter.formatDuration()`
  - 测试：传入秒值，验证格式化输出正确
...
- [ ] **T-4.5.1** `BackupExporter` 序列化 `reading_session` 表
  - 测试：插入 session 数据后导出，验证导出的 ZIP 中包含 `reading_session` 数据
  - 实现：新增表序列化

- [ ] **T-4.5.2** `BackupImporter` 反序列化 `reading_session` 表
...
- [ ] **T-5.1.1** `formatDuration()` — 0 秒
  - 测试：`formatDuration(0)` → `"0m"`
  - 实现：`< 60s → "0m"`

- [ ] **T-5.1.2** `formatDuration()` — 纯分钟
  - 测试：`formatDuration(1500)` → `"25m"`
  - 实现：`< 3600s → "Xm"`

- [ ] **T-5.1.3** `formatDuration()` — 小时 + 分钟
  - 测试：`formatDuration(5400)` → `"1h30m"`
  - 实现：`< 86400s → "XhYm"`

- [ ] **T-5.1.4** `formatDuration()` — 整小时省略分钟
  - 测试：`formatDuration(7200)` → `"2h"`
  - 实现：Y=0 时省略

- [ ] **T-5.1.5** `formatDuration()` — 天 + 小时
  - 测试：`formatDuration(90000)` → `"1d1h"`
  - 实现：`>= 86400s → "XdYh"`

- [ ] **T-5.1.6** `formatDuration()` — 整天省略小时
  - 测试：`formatDuration(86400)` → `"1d"`
  - 实现：Y=0 时省略

- [ ] **T-5.1.7** `formatWords()` — zh-CN 小于万
  - 测试：`formatWords(5000, zhLocale)` → `"5000字"`
  - 实现：`< 10000 → "X字"`

- [ ] **T-5.1.8** `formatWords()` — zh-CN 大于等于万
  - 测试：`formatWords(11840000, zhLocale)` → `"≈1184万"`
  - 实现：`>= 10000 → "≈X.X万"`

- [ ] **T-5.1.9** `formatWords()` — en 小于千
  - 测试：`formatWords(500, enLocale)` → `"500"`
  - 实现：`< 1000 → "X"`

- [ ] **T-5.1.10** `formatWords()` — en 大于等于千
  - 测试：`formatWords(1500, enLocale)` → `"≈1.5K"`
  - 实现：`>= 1000 → "≈X.XK"`

- [ ] **T-5.1.11** `formatPercent()` — 整数
  - 测试：`formatPercent(75.0f)` → `"75%"`
  - 实现：整数不带小数

- [ ] **T-5.1.12** `formatPercent()` — 非整数
  - 测试：`formatPercent(75.3f)` → `"75.3%"`
  - 实现：保留一位小数

- [ ] **T-5.1.13** `zeroOrNull()` — 零值
  - 测试：`zeroOrNull(0)` → `"--"`
  - 实现：0 → `"--"`

- [ ] **T-5.1.14** `zeroOrNull()` — 非零值
...
- [ ] **T-5.3.1** `next(DAY, ...)` 返回下一天
  - 测试：2026-06-09 → 2026-06-10
  - 实现：`next()` 方法

- [ ] **T-5.3.2** `prev(DAY, ...)` 返回前一天
  - 测试：2026-06-09 → 2026-06-08
  - 实现：`prev()` 方法

- [ ] **T-5.3.3** `next(WEEK, ...)` 返回下一周
  - 测试：验证加 7 天
  - 实现：`next()` 方法

- [ ] **T-5.3.4** `prev(WEEK, ...)` 返回上一周
  - 测试：验证减 7 天
  - 实现：`prev()` 方法

- [ ] **T-5.3.5** `next(MONTH, ...)` 返回下一月
  - 测试：2026-01-15 → 2026-02-15
  - 实现：`next()` 方法

- [ ] **T-5.3.6** `prev(MONTH, ...)` 返回上一月
  - 测试：2026-03-15 → 2026-02-15
  - 实现：`prev()` 方法

- [ ] **T-5.3.7** `next(YEAR, ...)` 返回下一年
  - 测试：2026-06-09 → 2027-06-09
  - 实现：`next()` 方法

- [ ] **T-5.3.8** `prev(YEAR, ...)` 返回上一年
...
- [ ] **T-8.1.2** 空数据状态显示 EmptyStatsState
...
- [ ] **T-8.3.4** `canGoNext = false` 时前进箭头禁用
  - 测试：前进箭头不可点击
  - 实现：`enabled` 状态
...
- [ ] **T-8.7.1** 渲染 3 行 × 8 列网格
  - 测试：传入 24 元素列表，渲染 24 格
  - 实现：`HourlyHeatmap.kt`

- [ ] **T-8.7.2** 小时轴标注（0/8/16）
  - 测试：验证左侧行标签显示 "0"/"8"/"16"
  - 实现：行标签
...
- [ ] **T-8.9.2** SegControl 切换维度
  - 测试：点击 "按格式"，回调 `onDimensionChange(FORMAT)` 触发
  - 实现：维度切换

- [ ] **T-8.9.3** TXT/EPUB 语义色
...
- [ ] **T-8.10.1** 渲染 Top 5 列表
  - 测试：传入 5 本书数据，LazyColumn 显示 5 行
  - 实现：`TopNList.kt`

- [ ] **T-8.10.2** SegControl 切换排序
...
- [ ] **T-8.12.5** 合并后 durationSeconds 为 SUM
...
- [ ] **T-9.3.1** `AppStrings` 新增 `stats` 字段
...
- [ ] **T-10.1.1** `ActiveScreen.Stats` 存在
  - 测试：`ActiveScreen.Stats` 可实例化，类型为 `ActiveScreen`
  - 实现：`data object Stats : ActiveScreen()`

### 10.2 BookshelfScreen 签名改造

- [ ] **T-10.2.1** `BookshelfScreen` 接收 `onNavigateToStats` 参数
  - 测试：编译通过，参数类型为 `() -> Unit`
  - 实现：新增参数

- [ ] **T-10.2.2** 统计图标按钮点击调用 `onNavigateToStats`
  - 测试：点击统计按钮，`onNavigateToStats()` 被调用
  - 实现：按钮回调

### 10.3 MainActivity 导航

- [ ] **T-10.3.1** `ActiveScreen.Stats` 分支渲染 `StatsScreen`
...
- [ ] **T-10.4.1** `ShuLiAppContainer.statsRepository` 可访问
  - 测试：`container.statsRepository` 不为 null，类型正确
  - 实现：lazy 属性

- [ ] **T-10.4.2** `ShuLiDatabase.readingSessionDao()` 可访问
...
- [ ] **T-11.3.1** 365 天热力图 Canvas 绘制 < 2ms
  - 测试：Benchmark 365 格 `drawRoundRect` 耗时
  - 实现：Canvas 优化

- [ ] **T-11.3.2** 全年 `getDailyTotals()` 查询 < 1ms
...
- [ ] **T-11.4.1** 热力图 contentDescription
  - 测试：TalkBack 朗读 "2026年6月9日，阅读45分钟"
  - 实现：`contentDescription`

- [ ] **T-11.4.2** Top N 列表项 contentDescription
...
- [ ] **T-A.1.1** `createReadingSession()` 工厂方法
  - 提供默认值的 `ReadingSessionEntity` 创建方法，支持参数覆盖

- [ ] **T-A.1.2** `createDailyTotalTuple()` 工厂方法
  - 快速创建 `DailyTotalTuple` 测试数据

- [ ] **T-A.1.3** `createBookWithDuration()` 工厂方法
  - 快速创建 `BookWithDurationTuple` 测试数据

- [ ] **T-A.1.4** `MockReadingSessionDao` 辅助
  - 预设常用 mock 行为的 DAO mock

### A.2 androidTest 数据库辅助

- [ ] **T-A.2.1** 扩展 `TestDatabaseFactory` 支持 `readingSessionDao()`
  - 确保内存数据库包含新表
