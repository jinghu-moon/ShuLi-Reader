...
/**
 * ModalBottomSheet 适配层：把 [SettingsPanelV5SheetContent] 嵌入 ModalBottomSheet，
...
private fun bridgeSettingChange(key: String, value: Any): ReaderIntent? {
    ...
    return when (key) {
        ...
        // ── 屏幕与亮度 ──
        "brightness" -> f(ReaderSettingKey.BRIGHTNESS)
        "color_temperature" -> f(ReaderSettingKey.COLOR_TEMPERATURE)
        "focus_line" -> b(ReaderSettingKey.FOCUS_LINE)
        "auto_night_mode" -> b(ReaderSettingKey.AUTO_NIGHT_MODE)
        // ── 显示模式 ──
